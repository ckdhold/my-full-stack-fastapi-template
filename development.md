...
Backend, JSON based web API based on OpenAPI: <http://localhost:6279>

Automatic interactive documentation with Swagger UI (from the OpenAPI backend): <http://localhost:6279/docs>
...
For the backend and frontend, they use the same port that would be used by their local development server, so, the backend is at `http://localhost:6279` and the frontend at `http://localhost:5173`.
...
Backend: <http://localhost:6279>

Automatic Interactive Docs (Swagger UI): <http://localhost:6279/docs>

Automatic Alternative Docs (ReDoc): <http://localhost:6279/redoc>
